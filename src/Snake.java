...
public class Snake extends Board {
    ...
    private Snake(boolean init) {
        super(init);
    } //bc there is no parameterless constructor in board u must use super
    ...
    private static boolean isCheck() {
        ...
        if (posLocal <= 0) {
            posLocal = 1;
        } //prevent invalid cells or negative values
    ...
    static void changeDirection(int key) {
        ...
        if (position <= Main.INT_CONSTANTS.CELL_COUNT.value && position > 0) {
            updateMovement();
        } else {
            Main.GameManager.gameStatus = false;
        }
    }
